...
def main(argv):
    ...
    hist_trk_pt_b = TH1D("trk_pt_b", ";Track q/p_{T} [1/GeV];Normalized entries", 20, -1/track_pt_bound[0], 1/track_pt_bound[0])
    hist_trk_pt_btoc = TH1D("trk_pt_btoc", ";Track q/p_{T} [1/GeV];Normalized entries", 20, -1/track_pt_bound[0], 1/track_pt_bound[0])
    hist_trk_pt_c = TH1D("trk_pt_c", ";Track q/p_{T} [1/GeV];Normalized entries", 20, -1/track_pt_bound[0], 1/track_pt_bound[0])
    hist_trk_pt_nohf = TH1D("trk_pt_nohf", ";Track q/p_{T} [1/GeV];Normalized entries", 20, -1/track_pt_bound[0], 1/track_pt_bound[0])
    hist_trk_theta_b = TH1D("trk_theta_b", ";Track #theta;Normalized entries", 20, 0, math.pi)
    hist_trk_theta_btoc = TH1D("trk_theta_btoc", ";Track #theta;Normalized entries", 20, 0, math.pi)
    hist_trk_theta_c = TH1D("trk_theta_c", ";Track #theta;Normalized entries", 20, 0, math.pi)
    hist_trk_theta_nohf = TH1D("trk_theta_nohf", ";Track #theta;Normalized entries", 20, 0, math.pi)
    hist_trk_phi_b = TH1D("trk_phi_b", ";Track #phi;Normalized entries", 20, -math.pi, math.pi)
    hist_trk_phi_btoc = TH1D("trk_phi_btoc", ";Track #phi;Normalized entries", 20, -math.pi, math.pi)
    hist_trk_phi_c = TH1D("trk_phi_c", ";Track #phi;Normalized entries", 20, -math.pi, math.pi)
    hist_trk_phi_nohf = TH1D("trk_phi_nohf", ";Track #phi;Normalized entries", 20, -math.pi, math.pi)
    hist_trk_d0_b = TH1D("trk_d0_b", ";Track d0 [mm];Normalized entries", 20, -track_d0_bound, track_d0_bound)
    hist_trk_d0_btoc = TH1D("trk_d0_btoc", ";Track d0 [mm];Normalized entries", 20, -track_d0_bound, track_d0_bound)
    hist_trk_d0_c = TH1D("trk_d0_c", ";Track d0 [mm];Normalized entries", 20, -track_d0_bound, track_d0_bound)
    hist_trk_d0_nohf = TH1D("trk_d0_nohf", ";Track d0 [mm];Normalized entries", 20, -track_d0_bound, track_d0_bound)
    hist_trk_z0_b = TH1D("trk_z0_b", ";Track z0 [mm];Normalized entries", 20, -track_z0_bound, track_z0_bound)
    hist_trk_z0_btoc = TH1D("trk_z0_btoc", ";Track z0 [mm];Normalized entries", 20, -track_z0_bound, track_z0_bound)
    hist_trk_z0_c = TH1D("trk_z0_c", ";Track z0 [mm];Normalized entries", 20, -track_z0_bound, track_z0_bound)
    hist_trk_z0_nohf = TH1D("trk_z0_nohf", ";Track z0 [mm];Normalized entries", 20, -track_z0_bound, track_z0_bound)

    #histograms of basic jet features by jet flavor
    bin_edges = np.linspace(-0.5,ntrk_bound+0.5,ntrk_bound+2)
    hist_no_trk_b = TH1D("no_trk_b", ";Number of tracks;Normalized entries", ntrk_bound+1, bin_edges)
    hist_no_trk_c = TH1D("no_trk_c", ";Number of tracks;Normalized entries", ntrk_bound+1, bin_edges)
    hist_no_trk_l = TH1D("no_trk_l", ";Number of tracks;Normalized entries", ntrk_bound+1, bin_edges)
    hist_jet_pt_b = TH1D("jet_pt_b", ";Jet pT [GeV];Normalized entries", 20, jet_pt_bound[0], jet_pt_bound[1])
    hist_jet_pt_c = TH1D("jet_pt_c", ";Jet pT [GeV];Normalized entries", 20, jet_pt_bound[0], jet_pt_bound[1])
    hist_jet_pt_l = TH1D("jet_pt_l", ";Jet pT [GeV];Normalized entries", 20, jet_pt_bound[0], jet_pt_bound[1])
    hist_jet_eta_b = TH1D("jet_eta_b", ";Jet #eta;Normalized entries", 20, -jet_eta_bound, jet_eta_bound)
    hist_jet_eta_c = TH1D("jet_eta_c", ";Jet #eta;Normalized entries", 20, -jet_eta_bound, jet_eta_bound)
    hist_jet_eta_l = TH1D("jet_eta_l", ";Jet #eta;Normalized entries", 20, -jet_eta_bound, jet_eta_bound)
    hist_jet_phi_b = TH1D("jet_phi_b", ";Jet #phi;Normalized entries", 20, -math.pi, math.pi)
    hist_jet_phi_c = TH1D("jet_phi_c", ";Jet #phi;Normalized entries", 20, -math.pi, math.pi)
    hist_jet_phi_l = TH1D("jet_phi_l", ";Jet #phi;Normalized entries", 20, -math.pi, math.pi)

    #histograms of track error features by track label
    if incl_errors:
        hist_trk_p_err_b = TH1D("trk_p_err_b", ";Track 1/p error [1/GeV];Normalized entries", 20, 0, 1./track_pt_err_bound[0])
        hist_trk_p_err_btoc = TH1D("trk_p_err_btoc", ";Track 1/p error [1/GeV];Normalized entries", 20, 0, 1./track_pt_err_bound[0])
        hist_trk_p_err_c = TH1D("trk_p_err_c", ";Track 1/p error [1/GeV];Normalized entries", 20, 0, 1./track_pt_err_bound[0])
        hist_trk_p_err_nohf = TH1D("trk_p_err_nohf", ";Track 1/p error [1/GeV];Normalized entries", 20, 0, 1./track_pt_err_bound[0])        
        hist_trk_theta_err_b = TH1D("trk_theta_err_b", ";Track #theta error;Normalized entries", 20, 0, track_theta_err_bound)
        hist_trk_theta_err_btoc = TH1D("trk_theta_err_btoc", ";Track #theta error;Normalized entries", 20, 0, track_theta_err_bound)
        hist_trk_theta_err_c = TH1D("trk_theta_err_c", ";Track #theta error;Normalized entries", 20, 0, track_theta_err_bound)
        hist_trk_theta_err_nohf = TH1D("trk_theta_err_nohf", ";Track #theta error;Normalized entries", 20, 0, track_theta_err_bound)
        hist_trk_phi_err_b = TH1D("trk_phi_err_b", ";Track #phi error;Normalized entries", 20, 0, track_phi_err_bound)
        hist_trk_phi_err_btoc = TH1D("trk_phi_err_btoc", ";Track #phi error;Normalized entries", 20, 0, track_phi_err_bound)
        hist_trk_phi_err_c = TH1D("trk_phi_err_c", ";Track #phi error;Normalized entries", 20, 0, track_phi_err_bound)
        hist_trk_phi_err_nohf = TH1D("trk_phi_err_nohf", ";Track #phi error;Normalized entries", 20, 0, track_phi_err_bound)
        hist_trk_d0_err_b = TH1D("trk_d0_err_b", ";Track d0 error [mm];Normalized entries", 20, 0, track_d0_err_bound)
        hist_trk_d0_err_btoc = TH1D("trk_d0_err_btoc", ";Track d0 error [mm];Normalized entries", 20, 0, track_d0_err_bound)
        hist_trk_d0_err_c = TH1D("trk_d0_err_c", ";Track d0 error [mm];Normalized entries", 20, 0, track_d0_err_bound)
        hist_trk_d0_err_nohf = TH1D("trk_d0_err_nohf", ";Track d0 error [mm];Normalized entries", 20, 0, track_d0_err_bound)
        hist_trk_z0_err_b = TH1D("trk_z0_err_b", ";Track z0 error [mm];Normalized entries", 20, 0, track_z0_err_bound)
        hist_trk_z0_err_btoc = TH1D("trk_z0_err_btoc", ";Track z0 error [mm];Normalized entries", 20, 0, track_z0_err_bound)
        hist_trk_z0_err_c = TH1D("trk_z0_err_c", ";Track z0 error [mm];Normalized entries", 20, 0, track_z0_err_bound)
        hist_trk_z0_err_nohf = TH1D("trk_z0_err_nohf", ";Track z0 error [mm];Normalized entries", 20, 0, track_z0_err_bound)

    #histograms of Lxy by vertex label (same as track label except btoc -> b)
    hist_vertex_lxy_b = TH1D("vertex_lxy_b", ";Vertex Lxy [mm];Normalized entries", 20, 0, lxy_bound)
    hist_vertex_lxy_c = TH1D("vertex_lxy_c", ";Vertex Lxy [mm];Normalized entries", 20, 0, lxy_bound)

    #histograms of track cuts as a function of basic track features
    hist_acc_trk_pt_hf = TH1D("acc_trk_pt_hf", ";Track pT [GeV];Entries", 20, track_pt_bound[0], track_pt_bound[1])
    hist_acc_trk_pt_nohf = TH1D("acc_trk_pt_nohf", ";Track pT [GeV];Entries", 20, track_pt_bound[0], track_pt_bound[1])
    hist_rej_trk_pt_hf = TH1D("rej_trk_pt_hf", ";Track pT [GeV];Entries", 20, track_pt_bound[0], track_pt_bound[1])
    hist_rej_trk_pt_nohf = TH1D("rej_trk_pt_nohf", ";Track pT [GeV];Entries", 20, track_pt_bound[0], track_pt_bound[1])
    hist_acc_trk_z0_hf = TH1D("acc_trk_z0_hf", ";Track z0 [mm];Entries", 20, -track_z0_bound, track_z0_bound)
    hist_acc_trk_z0_nohf = TH1D("acc_trk_z0_nohf", ";Track z0 [mm];Entries", 20, -track_z0_bound, track_z0_bound)
    hist_rej_trk_z0_hf = TH1D("rej_trk_z0_hf", ";Track z0 [mm];Entries", 20, -track_z0_bound, track_z0_bound)
    hist_rej_trk_z0_nohf = TH1D("rej_trk_z0_nohf", ";Track z0 [mm];Entries", 20, -track_z0_bound, track_z0_bound)
    ...
    plot_hist(canv, [hist_acc_trk_z0_hf, hist_rej_trk_z0_hf], ["passed", "cut"], cut_string, False, True, data_path+data_name+"_cut_trk_z0_hf.png")
    plot_hist(canv, [hist_acc_trk_z0_nohf, hist_rej_trk_z0_nohf], ["passed", "cut"], cut_string, False, True, data_path+data_name+"_cut_trk_z0_nohf.png")
    plot_hist(canv, [hist_acc_trk_lxy_b, hist_rej_trk_lxy_b], ["passed", "cut"], cut_string, False, True, data_path+data_name+"_cut_trk_lxy_b.png")
    plot_hist(canv, [hist_acc_trk_lxy_c, hist_rej_trk_lxy_c], ["passed", "cut"], cut_string, False, True, data_path+data_name+"_cut_trk_lxy_c.png")
    plot_bar(canv, [hist_acc_trk_id_b, hist_rej_trk_id_b], b_pdgids, ["passed", "cut"], cut_string, False, True, data_path+data_name+"_trk_ancid_b.png")
    plot_bar(canv, [hist_acc_trk_id_c, hist_rej_trk_id_c], c_pdgids, ["passed", "cut"], cut_string, False, True, data_path+data_name+"_trk_ancid_c.png")
    plot_bar(canv, [hist_acc_trk_id_btoc, hist_rej_trk_id_btoc], b_pdgids, ["passed", "cut"], cut_string, False, True, data_path+data_name+"_trk_ancid_btoc.png")

    if incl_vweight:
        plot_hist(canv, [hist_acc_trk_vweight_hf, hist_rej_trk_vweight_hf], ["passsed", "cut"], cut_string, False, True, data_path+data_name+"_cut_trk_vweight_hf.png")
        plot_hist(canv, [hist_acc_trk_vweight_nohf, hist_rej_trk_vweight_nohf], ["passsed", "cut"], cut_string, False, True, data_path+data_name+"_cut_trk_vweight_nohf.png")
