...
def main(argv):
    ...
    parser.add_argument("-d", "--dataset", type=str, required=True, dest="outfile_name", help="name of dataset for which outputs are generated (without hdf5 extension)")
    parser.add_argument("-n", "--ntuples", type=str, required=True, dest="ntuples", help="name of ntuples to be processed")
    ...
    parser.add_argument("-i", "--data_dir", type=str, required=True, dest="data_dir", help="name of directory where input data is stored")
    ...
    if only_bad_events and os.path.isfile(registerfilename):
        ...
        for line in contents:
            ...
            file = str(int(float(values[0])))
            event = str(int(float(values[1])))
            jet = str(int(float(values[2])))
            key = file+"."+event
            if key in bad_events.keys():
                bad_events[key] = np.append(bad_events[key], jet)
            else:
                bad_events[key] = np.array([jet])
    ...
    ifile = 0
    total_processed_jets = 0
    for ntuple in ntuples:
        file = TFile(data_dir+ntuple+".root") 
        tree = file.Get(args.tree)
        file_processed_jets = 0

        for ientry,entry in enumerate(tree):
            key = str(ifile)+"."+str(ientry)

            if not only_bad_events and file_processed_jets>=maxentries: continue

            #only process events in the register if only_bad_events is true
            if not only_bad_events or key in bad_events.keys():
                ...
                for i in range(njets):
                    if (not only_bad_events or str(i) in bad_events[key]) and check_jet(entry, i, jet_pt_cut, jet_eta_cut):
                        rem_trk = 0
                ...
                for i in range( njets ):
                    ...
                    if (not only_bad_events or str(i) in bad_events[key]) and check_jet(entry, i, jet_pt_cut, jet_eta_cut):
                        ...
                        file_processed_jets += 1
                        total_processed_jets += 1
                        ...
                        #output progress
                        sys.stdout.write("\rProcessed {} jets. Time elapsed: {:.1f}s".format(total_processed_jets, time.time()-start_time))
                        sys.stdout.flush()
        
        ifile += 1

    sys.stdout.write("\rFinished processing. Total jets used from sample: {}".format(total_processed_jets))
    sys.stdout.flush()
    print("\nCreating plots")
